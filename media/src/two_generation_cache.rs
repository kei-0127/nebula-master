    ...
    K: Hash + Eq,
{
    ...
    pub fn remove_old(&mut self, now: Instant) -> Vec<K> {
        if let Some(mut removed_gen) =
            self.swap_generations_if_its_been_too_long(now)
        {
            removed_gen
                .drain()
                .map(|(k, _v)| k)
    ...
    fn swap_generations_if_its_been_too_long(
        &mut self,
        now: Instant,
    ) -> Option<HashMap<K, V>> {
        if now >= self.generation1_expires {
            let removed_gen = mem::replace(
                &mut self.generation1,
                mem::take(&mut self.generation0),
            );
...
mod two_generation_cache_tests {
    ...
    fn insert_and_get() {
        let mut lru =
            TwoGenerationCache::new(Duration::from_secs(1), Instant::now());
