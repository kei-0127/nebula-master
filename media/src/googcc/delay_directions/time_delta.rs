...
impl TimeDeltaSince for Instant {
    fn time_delta_since(self, other: Self) -> TimeDelta {
        if self > other {
            TimeDelta::from_secs(
                self.checked_duration_since(other).unwrap().as_secs_f64(),
            )
        } else {
            TimeDelta::from_secs(
                -other.checked_duration_since(self).unwrap().as_secs_f64(),
            )
        }
    }
}
...
impl TimeDeltaSince for crate::transportcc::RemoteInstant {
    fn time_delta_since(self, other: Self) -> TimeDelta {
        if self > other {
            ...
            TimeDelta::from_secs(
                -other.saturating_duration_since(self).as_secs_f64(),
            )
        }
    }
}
