...
impl AckGroupSummary {
    fn add(&mut self, ack: &Ack) {
        ...
        self.latest_feedback_arrival =
            max(self.latest_feedback_arrival, ack.feedback_arrival);
    }
}
...
mod accumulate_ack_groups_tests {
    ...
    pub(super) fn acks_from_departure_and_arrival(
        ...
        futures::stream::iter(pairs.into_iter().map(move |(departure, arrival)| {
            Ack {
    ...
    fn groups() {
        ...
        let g2_arrival_time =
            g1_arrival_time + BURST_THRESHOLD + Duration::from_millis(1);
        let g3_arrival_time =
            g2_arrival_time + BURST_THRESHOLD + Duration::from_millis(1);
        let g4_arrival_time =
            g3_arrival_time + BURST_THRESHOLD + Duration::from_millis(1);
    ...
    fn accumulated_group() {
        ...
        for _ in 0..10 {
            ...
            current_arrival_time += BURST_THRESHOLD + Duration::from_millis(1);
    ...
    fn max_within_groups() {
        ...
        let stream = accumulate_ack_groups(acks_from_departure_and_arrival(vec![
            ...
            (
                ...
                g2_arrival_time + Duration::from_millis(1),
            ),
            (start + NEW_GROUP_INTERVAL, g2_arrival_time),
            ...
            (
                ...
                g3_arrival_time + Duration::from_millis(1),
            ),
            (
            ...
            (
                ...
                g4_arrival_time + Duration::from_millis(1),
            ),
            (
                start + 3 * NEW_GROUP_INTERVAL + Duration::from_millis(1),
                g4_arrival_time,
            ),
            // One more to flush.
        ...
        assert_eq!(
            &[
                ...
                (
                    3 * NEW_GROUP_INTERVAL + Duration::from_millis(1),
                    g4_arrival_time + Duration::from_millis(1),
                    g5_arrival_time
                ),
            ],
            &stream
                .inspect(|(group, _ack)| {
                    assert_eq!(
                        group.latest_departure,
                        group.latest_feedback_arrival
                    );
                })
                .map(|(group, ack)| (
                    group
    ...
    fn negative_arrival_time_jump() {
        ...
        assert_eq!(
            &[
                ...
                (
                    first_arrival_after_jump
                        + (number_of_packets_after_jump as u32 - 1) * interval,
                    interval,
                    TimeDelta::from_secs(interval.as_secs_f64()),
                ),
            ],
...
pub fn calculate_delay_directions(
    ...
    calculate_delay_directions_from_slopes(calculate_delay_slopes(
        calculate_ack_deltas(accumulate_ack_groups(acks)),
    ))
}
...
mod calculate_delay_directions_from_slopes_tests {
    ...
    fn simple_overuse_2000_kbit_30_fps() {
        ...
        assert_eq!(
            ...
            stream
                .skip_while(|(_, direction)| ready(
                    *direction != DelayDirection::Increasing
                ))
                .map(|(instant, _)| instant
                    .checked_duration_since(initial_departure)
                    .unwrap())
                .next()
                .now_or_never()
                .unwrap()
    ...
    fn assert_increasing_after_drift_with_variance(
        ...
        let first_after_change = stream
            .skip_while(|(_, direction)| {
                ready(*direction != DelayDirection::Increasing)
            })
            .map(|(instant, _)| {
                instant.checked_duration_since(initial_departure).unwrap()
            })
            .next()
            .now_or_never()
        ...
        if let Some(first_after_change) = first_after_change {
            assert!(
                100_000 * frame_interval + RTT_FOR_ACKS_AT_REGULAR_INTERVALS
                    < first_after_change,
            );
        } else {
    ...
    fn threshold_adapts() {
        ...
        let stream =
            calculate_delay_directions_from_slopes(slope_tuples).chunks(batch_size);
        pin_mut!(stream);
    ...
    fn does_not_adapt_to_spikes() {
        ...
        let slope_tuples = slopes
            .zip(incrementing_times)
            .map(|(slope, now)| (now, slope, Duration::from_secs(3), 60));
        let stream_output: Vec<_> =
            calculate_delay_directions_from_slopes(slope_tuples)
                .collect()
                .now_or_never()
                .unwrap();
...
fn linear_regression(
    xys: impl ExactSizeIterator<Item = (f64, f64)> + Clone,
) -> Option<f64> {
